...
const wpChapters = [
  { id: 'wp-ch1', title: 'Executive summary', src: 'whitepaper.html#ch1', tags: ['summary','overview','positioning'],
    text: 'ONEWALLET is a Telegram-native Web3 wallet that replaces seed phrases with 2-of-3 Shamir custody. The whitepaper covers product, architecture, security, token model, growth, business model, roadmap, team, and risks.' },
  { id: 'wp-ch2', title: 'Product thesis — why Telegram-native finance', src: 'whitepaper.html#ch2', tags: ['thesis','telegram','mini app','stablecoin'],
    text: 'Telegram Mini Apps run native-feeling UIs without app-store gatekeeping. Shamir-based custody has matured from research to production-grade libraries. Stablecoin volume is structurally meaningful for merchant payments. Consumer fatigue with seed-phrase ceremonies opens the field for keyless wallets. ONEWALLET combines wallet, payment rail, and ecosystem in one Telegram-native flow.' },
  ...
  { id: 'wp-ch4', title: 'Technical architecture — four layers', src: 'whitepaper.html#ch4', tags: ['architecture','edge','custody','settlement'],
    text: 'Layer 1 Client: Telegram Mini App, React + TMA SDK. Layer 2 Edge: auth, Shamir signing orchestration, price feeds, webhooks; stateless functions that hold no share; the key is reconstructed only transiently on the user device during signing. Layer 3 Custody (target architecture): signer service, recovery share registry, Telegram-share binding; hardened environment; planned hardware-backed share protection; holds no single signable key. Layer 4 Settlement: chain adapters, payment processor, indexer, merchant API; TON and EVM; idempotent settlement.' },
  { id: 'wp-ch5', title: 'Security model — 2-of-3 Shamir and threat model', src: 'whitepaper.html#ch5', tags: ['security','shamir','multi-share','threshold','threat model','recovery'],
    text: 'Custody promise: no single party can move user funds. The 2-of-3 Shamir topology uses a device share, a Telegram-account-bound share, and a recovery share; any two are combined on the user device in an isolated context to reconstruct the key, sign, then wipe it; shares are rotatable. Threat model: device theft is mitigated by rotating the device share via the recovery share plus a fresh Telegram-bound share; a Telegram account or session hijack cannot sign because the Telegram share alone is insufficient — an attacker still needs a second share (device or recovery) — and high-value transfers add re-auth and cooldown; a coerced recovery attempt — where a user is tricked or pressured into using the recovery share — is mitigated by a time-locked recovery window with multi-step confirmation, surfacing alerts before any share is released; a signing attempt from a compromised device, where malware on the device tries to combine two shares, is met with per-signature biometric re-auth, value caps, cooldowns, and anomaly checks that raise attacker cost and surface intervention before high-value broadcasts. Because Shamir reconstructs the key at signing time, there is a brief on-device exposure window, mitigated by ephemeral reconstruction and immediate wipe. No custody model is unbreakable.' },
  ...
  { id: 'wp-ch9', title: 'Roadmap — public status, no marketing dates', src: 'whitepaper.html#ch9', tags: ['roadmap','phases','status'],
    text: 'Phase 1 Core wallet (Multi-Share custody, TMA, multi-chain): completed. Phase 2 Payments rail (QR invoices, settlement webhooks): in progress. Phase 3 $1 utility (fee discount layer, reward engine): planned. Phase 4 Ecosystem (mini-app store, partner program): planned. Dates are intentionally omitted until owners approve public commitments.' },
	  { id: 'wp-ch10', title: 'Team and governance', src: 'whitepaper.html#ch10', tags: ['team','governance','founders'],
	    text: 'ONE COMPANY is listed as a Malaysia foundation registered with SSM, Companies Commission of Malaysia. Business address: 3F. KLTS Block C No78, Jalan Gombak, Kuala Lumpur. Business nature: a global digital platform business that integrates and provides digital wallet, fintech solutions, games, AI, travel, and information services based on blockchain technology. Business objectives cover blockchain and Web3 research, education, and support; digital wallet, fintech, AI, game, travel, information platform ecosystems; blockchain platform development, operation, distribution, and standardization; healthy digital asset and token use; Web3 community, membership, and rewards; AI automation and data platforms; online games, digital content, NFT and metaverse ecosystems; travel, tourism, payment, and membership platforms; partnerships, events, publishing, youth and startup education, fundraising, sponsorship, public-interest R&D, project support, safety, transparency, user protection, and compliance systems. Public team roles: James Kim CEO and project leader; Kim Jae Mo Director CMO; Max Khoo Director CFO; Lee Jun Bong Director; Park Ju-Jin Director. Park Ju-Jin previous roles listed in the foundation update: Former Chairman of SearchOne Co., Ltd.; Former CEO of Tongil Trading Co., Ltd.; Former CEO of HighGood Co., Ltd.; Former Chairman of CrossCoin Foundation.' },
  { id: 'wp-ch11', title: 'Risks and mitigations', src: 'whitepaper.html#ch11', tags: ['risk','mitigation','custody','platform','regulatory'],
    text: 'High: custody compromise — Shamir reconstructs the key at signing time, creating a brief in-memory exposure window on the device; any flaw in share rotation, reconstruction, or key wipe could expose funds. High: platform dependency — Telegram is the runtime; progressive web fallback is planned. Medium: regulatory shift — rules vary by region; legal counsel is the mitigation. Medium: merchant adoption — requires integration density; anchor partners. Low: token utility drift — activity-linked rewards. Low: localization quality — native review pipeline.' },
  { id: 'wp-ch12', title: 'Glossary and references', src: 'whitepaper.html#ch12', tags: ['glossary','definitions','vocabulary','shamir','multi-share','tma','webhook'],
    text: "Shamir's Secret Sharing (SSS): splits a key into N shares so any K (here 2 of 3) can reconstruct it while fewer reveal nothing; ONEWALLET reconstructs the key only briefly, on-device in an isolated context, to sign, then wipes it. Threshold scheme (Shamir-based): a K-of-N access structure; not a threshold signature scheme (TSS), which never reconstructs the key. TMA: Telegram Mini App. Settlement webhook: a signed HTTP callback used for merchant reconciliation. Recovery share: an offline Shamir share used in recovery flows." }
];
...
const homepageSections = [
  { id: 'hp-platform', title: 'Platform — a complete stack for Telegram-native money', src: 'index.html#platform', tags: ['platform','stack','overview'],
    text: 'Open a wallet inside a chat. QR rail, not a portfolio app. 2-of-3 Shamir, no persistent single point of failure. Web3 acceptance with Web2 ergonomics. $1 powers the working layer.' },
  { id: 'hp-audience', title: 'Audiences — users, investors, merchants', src: 'index.html#audience', tags: ['audience','users','investors','merchants'],
    text: 'For users: a wallet that just opens. Keyless onboarding inside Telegram. Plain-language recovery. Send, pay, and earn without managing a seed phrase. For investors: Telegram-scale distribution, Multi-Share custody architecture, token utility tied to real activity, referral-driven growth. For merchants: Web3 payments without the Web3 work — QR invoicing, stablecoin settlement, webhooks, ecosystem rewards.' },
  { id: 'hp-security', title: 'Security — keyless by design, recoverable by default', src: 'index.html#security', tags: ['security','shamir','multi-share','recovery','audit'],
